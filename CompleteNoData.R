...
dat<-list(datapoints=nrow(reshapeddata), virusspecies=length(unique(reshapeddata$Virus)),hostspecies=length(unique(reshapeddata$Host)),
          ...
          run_estimation=0, host_pred=as.numeric(as.factor(preddataset[,1])),virus_pred=as.numeric(as.factor(preddataset[,2])),
...
save(fit, file="~/Exp1/DoublePhyloStan/Results/NoData.Rdata")
